...
static void *gfx_ctx_qnx_init(void *video_driver)
{
   ...
   {
      RARCH_ERR("screen_get_window_property_iv [SCREEN_PROPERTY_SIZE] failed with errno %d (%s).\n", errno, strerror(errno));
      goto error;
   }
   ...
   {
      RARCH_ERR("screen_get_display_property_pv [SCREEN_PROPERTY_MODE] failed with errno %d (%s).\n", errno, strerror(errno));
      goto error;
   }
...
static int dpi_get_density(qnx_ctx_data_t *qnx)
{
   ...
   {
      RARCH_ERR("screen_get_display_property_iv [SCREEN_PROPERTY_DPI] failed with errno %d (%s).\n", errno, strerror(errno));
      return -1;
   }
